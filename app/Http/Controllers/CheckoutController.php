	...
	public function orderSuccess($id, Request $request) {
		...
		try {
			...
			if ($erp_api) {
				...
				$existing_customer = Http::withHeaders($headers)
				  	->get($erp_api->base_url . '/api/resource/Customer/' . $customer_name);

				if ($existing_customer->failed()) {
					// save customer in erp
					$customer_data = [
						'customer_name' => $customer_name,
						'customer_type' => 'Individual',
						'customer_group' => 'End User',
						'territory' => $order_details->order_bill_city
					];

					$customer_insert_response = Http::withHeaders($headers)
						->post($erp_api->base_url . '/api/resource/Customer', ($customer_data));

					if ($customer_insert_response->successful()) {
						$customer_name = $customer_insert_response['data']['name'];
					}
		
					$billing_address = [
						'address_type' => 'Billing',
						'address_line1' => $order_details->order_bill_address1,
						'address_line2' => $order_details->order_bill_address2,
						'city' => $order_details->order_bill_brgy . ', ' . $order_details->order_bill_city,
						'state' => $order_details->order_bill_prov,
						'country' => $order_details->order_bill_country,
						'email_id' => $order_details->order_bill_email,
						'phone' => $order_details->order_bill_contact,
						'links' => [[
					...
					Http::withHeaders($headers)
						->post($erp_api->base_url . '/api/resource/Address', ($billing_address));

					$shipping_address = [
						'address_type' => 'Shipping',
						'address_line1' => $order_details->order_ship_address1,
						'address_line2' => $order_details->order_ship_address2,
						'city' => $order_details->order_ship_brgy . ', ' . $order_details->order_ship_city,
						'state' => $order_details->order_ship_prov,
						'country' => $order_details->order_ship_country,
						'email_id' => $order_details->order_email,
						'phone' => $order_details->order_contact,
						'links' => [[
							'link_doctype' => 'Customer',
							'link_name' => $customer_name,
							'link_title' => $customer_name,
					...
					Http::withHeaders($headers)
						->post($erp_api->base_url . '/api/resource/Address', ($shipping_address));

					$contact_person = [
						'first_name' => $order_details->order_ship_contactperson,
						'links' => [[
							'link_doctype' => 'Customer',
							'link_name' => $customer_name,
							'link_title' => $customer_name,
					...
					Http::withHeaders($headers)
						->post($erp_api->base_url . '/api/resource/Contact', ($contact_person));
				}
			}
