...
class Newsletter
{
    ...
    public function getTemplatesList(){
        ...
        $template_array = [];
        foreach(collect($templates) as $template){
            foreach(collect($template) as $a){
                $template_array[] = [
                    'template_id' => isset($a['id']) ? $a['id'] : null,
                    'template_type' => isset($a['type']) ? $a['type'] : null,
                    'template_name' => isset($a['name']) ? $a['name'] : null
                ];
            }
        }
    ...
    public function getSegmentsList($list_id){
        $segments = $this->mailChimp->get('/lists/'.$list_id.'/segments');

        $segments_array = [];
        foreach(collect($segments) as $segment){
            foreach(collect($segment) as $b){
                $segments_array[] = [
                    'list_id' => isset($b['id']) ? $b['id'] : null,
                    'list_name' => isset($b['name']) ? $b['name'] : null,
                    'list_type' => isset($b['type']) ? $b['type'] : null
                ];
            }
        }
    ...
    public function sendCampaign(){ // for testing
        $this->mailChimp->post('/campaigns/ddccf12644/actions/send');
    }
}
